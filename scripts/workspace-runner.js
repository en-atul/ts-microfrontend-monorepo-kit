...
if (!fs.existsSync(configPath)) {
  console.error('Missing workspace-runner.json');
  process.exit(1);
}
...
if (allPkgPaths.length === 0) {
  console.error('No packages found or no workspace configuration detected.');
  process.exit(1);
}
...
if (runningPackages.length === 0) {
  console.error('No running applications found with the specified command.');
  process.exit(1);
}
...
if (!enableUI) {
  ...
  runningPackages.forEach((pkgPath) => {
    const pkgJson = JSON.parse(fs.readFileSync(path.join(pkgPath, 'package.json'), 'utf-8'));
    const name = pkgJson.name;
    console.log(`Running ${command} in ${name}`);
